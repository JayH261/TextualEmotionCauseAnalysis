...
class Decoder(torch.nn.Module):
    def __init__(self, encoder_hidden_size=100, decoder_hidden_size=100, attention_hidden_size=100, num_classes=10,
        ...
        # multihead attention layer - our implementation
        self.attention = self.attention = nn.MultiheadAttention(embed_dim=50, num_heads=5,
                                                                dropout=0.2)  # take in q, k, v
        ...
        # Luong proposed global context attention layer - original
    ...
    def forward(self, inputs, last_hidden, encoder_outputs, current_encoder_outputs, time_step, max_len, mask,
        ...
        # For luong attention layer - original authors
        ...
        output = F.leaky_relu(self.linear(output))  # final representation for the target clause
...
class Network(nn.Module):
    def __init__(self, configs):
        ...
        self.bilstm = nn.LSTM(768, 384, num_layers=2, batch_first=True, bidirectional=True)
    ...
    def forward(self, bert_token_b, bert_segment_b, bert_masks_b,
        ...
        pred_e = self.seq(lstm_output, doc_len, epoch, testing, target)
        return pred_e
