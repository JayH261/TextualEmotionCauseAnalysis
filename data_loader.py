...
def build_training_data(configs, fold_id):
    train_dataset = CustomDataset(configs, fold_id, "train")
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #print("CONVERSATION ID, doc_len, len y emotions:", (doc_id,doc_len,len(y_emotions)))
...
def bert_batch_preprocessing(batch):
    convo_couples_b, y_emotions_b, y_causes_b, convo_len_b, convo_id_b, \
    bert_token_b, bert_segment_b, bert_utterance_b, bert_token_lens_b = zip(*batch)

    y_mask_b, y_emotions_b, y_causes_b = pad_docs(convo_len_b, y_emotions_b, y_causes_b)
    ...
    bert_utterance_b = pad_sequence(bert_utterance_b, batch_first=True, padding_value=0)
    ...
    return np.array(convo_len_b), np.array(adj_b), \
           np.array(y_emotions_b), np.array(y_causes_b), np.array(y_mask_b), convo_couples_b, convo_id_b, \
           bert_token_b, bert_segment_b, bert_masks_b, bert_utterance_b
